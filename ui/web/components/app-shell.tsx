        ...
        <div className="pointer-events-none sticky top-5 z-40 flex justify-center px-4 pb-4">
          <div className="pointer-events-auto flex h-12 items-center gap-2 rounded-full border border-white/10 bg-foreground/[0.55] p-1.5 pl-4 shadow-[0_12px_40px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.14)] backdrop-blur-2xl">
            ...
            >
              <TabsTrigger
                value="chat"
                className="rounded-full data-active:bg-foreground data-active:text-background"
              >
                <MessageCircle /> Chat
              </TabsTrigger>
              <TabsTrigger
            ...
            {provider ? (
              <span
                className="flex items-center gap-2 rounded-full px-3 py-1.5 font-mono text-xs text-foreground/80"
              ...
              <button
                onClick={() => { setTab("llm"); scrollToApp() }}
                className="rounded-full px-3 py-1.5 font-mono text-xs text-foreground/50 underline-offset-4 transition-colors hover:text-foreground hover:underline"
              >
                no llm active
              </button>
            )}
          ...
          {stats && (
            <div className="border-y border-white/[0.06] py-12">
              <div className="mx-auto flex max-w-[1000px] flex-wrap items-center justify-center gap-x-12 gap-y-4 font-mono text-[11px] uppercase tracking-[0.22em] text-foreground/30">
                <span>
                  <span className="mr-2 text-foreground/60">{stats.total_documents}</span>
                  documents
                </span>
                <span>
                  <span className="mr-2 text-foreground/60">{stats.total_chunks}</span>
                  chunks indexed
                </span>
                <span>
                  <span className="mr-2 text-foreground/60">{stats.embedding_dimension}D</span>
                  embeddings
                </span>
                <span>
                  <span className="mr-2 text-foreground/60">{stats.total_size_kb}</span>
                  KB stored
                </span>
              </div>
            </div>
